...
export const getGroupByExpression = ({
  ...
  const shouldHaveTimeZone =
    GROUP_BY_DATE_GRANULARITY_THAT_REQUIRE_TIME_ZONE.includes(dateGranularity);

  const timeZoneIsNotProvided = !isNonEmptyString(groupByField.timeZone);

  if (shouldHaveTimeZone && timeZoneIsNotProvided) {
  ...
  switch (dateGranularity) {
    ...
    case ObjectRecordGroupByDateGranularity.WEEK: {
      ...
      let shiftedExpression = `DATE_TRUNC('week', ${columnNameWithQuotes}, '${groupByField.timeZone}')`;

      if (isDefined(weekStartDay)) {
        if (weekStartDay === 'SUNDAY') {
          shiftedExpression = `DATE_TRUNC('week', ${columnNameWithQuotes} + INTERVAL '1 day', '${groupByField.timeZone}') - INTERVAL '1 day'`;
        } else if (weekStartDay === 'SATURDAY') {
          shiftedExpression = `DATE_TRUNC('week', ${columnNameWithQuotes} + INTERVAL '2 days', '${groupByField.timeZone}') - INTERVAL '2 days'`;
        }
      }
